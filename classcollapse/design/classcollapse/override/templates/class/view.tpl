...
{* Build a hash of hash  : class attributes indexed on attribute identifier,*}
{* indexed on categories configured in content.ini *}

{def $class_attributes_grouped_data_map  = hash()}
{def $attributeCategory = ''}
{def $attributeIdentifier = ''}
{def $attributeArray = hash()}

{foreach $attributes as $classAttribute}
	{set $attributeCategory   = $classAttribute.category}
	{set $attributeIdentifier = $classAttribute.identifier}
	
	{if or(is_unset($attribute_categorys[$attributeCategory]),eq($attributeCategory,''))}
		{set $attributeCategory = $attribute_default_category}
	{/if}
	
	{if is_unset($class_attributes_grouped_data_map[$attributeCategory])}
		{set $class_attributes_grouped_data_map = $class_attributes_grouped_data_map|merge(hash($attributeCategory,array($classAttribute)))}
	{else}
		{set $attributeArray=$class_attributes_grouped_data_map[$attributeCategory]}
		{set $attributeArray=$attributeArray|merge(hash($attributeIdentifier,$classAttribute))}
		{set $class_attributes_grouped_data_map=$class_attributes_grouped_data_map|merge(hash($attributeCategory, $attributeArray))}
	{/if}
{/foreach}

{* If there is more than 1 category, display sections, otherwise standard display *}

{if gt($class_attributes_grouped_data_map|count(),1)}
	{include uri='design:class/view_collapse.tpl'}
{else}
